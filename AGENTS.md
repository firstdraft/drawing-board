...
- If `application/` already exists, preserve it. Never delete, overwrite, or move it merely to satisfy the absent-path
  ...
  approved remote, or preserve it as a sibling under `/workspaces` before starting a different Compile. Do not use
  `/tmp`, the container home, or another path that a Codespace rebuild discards.
