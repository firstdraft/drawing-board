...
`script/devcontainer-smoke` to verify that browser tests have not started Selenium during workspace setup.
Root-adopted applications instead use their generated `.devcontainer/compose.yaml` and the running container's
Compose project, as shown in the [browser-testing instructions](README.md#7-open-your-app). The generated health
check uses Selenium's supplied `/opt/bin/check-grid.sh`; Compose owns readiness for both that command and fresh
generated Dev Container startup.
