...
- Never request a GitHub personal access token. Direct output creates no Publication or repository. If the user
  ...
  private repository, not a branch or pull request in this Drawing Board.
...
- After direct Compile and initialization succeed, report `application/`, its nested initial commit, and its
  `.firstdraft/submitted-foundation-plan.json` and `.firstdraft/gaps.json`; continue inside that directory. After
  explicit Publication succeeds, report the validated private GitHub URL and continue only in a separate checkout
  when the user asks.
