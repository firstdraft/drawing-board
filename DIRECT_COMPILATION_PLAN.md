...
The intended result is the generated Foundation at the repository root, the existing root `.git` retained, and
preexisting non-secret design materials moved under `./design`. The transaction must classify private local state
before moving anything: `.env`, `.firstdraft/state.json`, and any other credential or concurrency state must never
be staged or committed. Moving Drawing Board's tracked `.gitignore` to `design/.gitignore` must preserve its
`.env`/`.firstdraft` protections within that subtree; before relocation the transaction must write and verify
`.git/info/exclude` entries for `/design/.env`, `/design/.firstdraft/`, and any other detected private paths, then
verify the final subtree exclusions before any Git staging. The generated root `.gitignore` and the two generated
artifact-owned `.firstdraft` files remain exact and trackable.

The command must define the remaining collisions for `.firstdraft`, `.gitignore`, `AGENTS.md`, and other artifact
paths; preserve root Git history; reject nested or linked paths, an existing `design` destination, and an unsafe
partial relocation; and never manufacture a parentless repository at the root. It must stage and validate the
artifact before moving anything, define rollback or a recoverable failure boundary, and preserve both the original
design bytes and the exact generated bytes. Qualification must prove that root Git sees the non-secret moved design
paths and generated Foundation paths while every credential/private-state path remains ignored and untracked and a
credential scan stays clean. There is no interactive prompt in this agent-first phase.
